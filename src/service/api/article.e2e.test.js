...
describe(`API returns a list of all articles`, () => {
  ...
  test(`Last article's title equals "Как перестать беспокоиться и начать жить"`, () => expect(response.body[4].title).toBe(`Как перестать беспокоиться и начать жить`));
...
describe(`API changes existent article`, () => {
  ...
  test(`article is really changed`, () => request(app)
    .get(`/articles/2`)
    .expect((res) => expect(res.body.title).toBe(`Дам погладить песика`))
  );
...
test(`API returns status code 404 when trying to change non-existent article`, async () => {
  ...
  const validArticle = {
    categories: [3],
    title: `Это вполне валидный объект`,
    fullText: `однако поскольку такоой статьи в базе нет`,
    announce: `мы получим 404`
  };
